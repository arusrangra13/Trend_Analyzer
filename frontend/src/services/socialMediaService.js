...
export class SocialMediaService {
  ...
  static calculateTotalMetrics(socialData) {
    ...
    socialData.forEach(data => {
      switch (data.platform) {
        case 'youtube':
          ...
          totals.totalEngagement += data.engagement || 0;
          totals.totalPosts += data.videos || 0;
          totals.growthRate += data.recentGrowth || 0;
          break;
        case 'instagram':
    ...
    if (socialData.length > 0) {
      ...
      totals.growthRate = totals.growthRate / socialData.filter(d => d.platform === 'youtube').length || 0;
    }
  ...
  static saveSocialData(socialData) {
  ...
  static loadSocialData() {
    try {
      const saved = localStorage.getItem('socialMediaData');
